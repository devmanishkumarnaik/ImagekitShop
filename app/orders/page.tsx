...
// //   const handleDownload = async (imageUrl: string, fileName: string) => {
// //     try {
// //       // Fetch the original image without any transformations
// //       const response = await fetch(imageUrl);
// //       const blob = await response.blob();
// //       const url = window.URL.createObjectURL(blob);
// //       const a = document.createElement("a");
...
// //                             onClick={() =>
// //                               handleDownload(
// //                                 `${process.env.NEXT_PUBLIC_URL_ENDPOINT}/${product.imageUrl}`,
// //                                 `image-${order._id?.toString().slice(-6)}.jpg`
// //                               )
// //                             }
...
//       const fileExtension = imageUrl.split(".").pop() || "jpg"; // Default to "jpg" if no extension is found
...
//       const a = document.createElement("a");
//       a.href = url;
//       a.download = fullFileName; // Use the full file name with the original extension
//       document.body.appendChild(a);
//       a.click();
//       a.remove();
//       window.URL.revokeObjectURL(url);
...
//                               handleDownload(
//                                 `${process.env.NEXT_PUBLIC_URL_ENDPOINT}/${product.imageUrl}`,
//                                 `image-${order._id?.toString().slice(-6)}`
//                               )
//                             }
//                             className="btn btn-primary gap-1"
//                           >
...
export default function OrdersPage() {
  ...
  const handleDownload = async (imageUrl: string, fileName: string) => {
    try {
      ...
      // Extract the original file extension from the Content-Type header
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `${fileName}.${fileExtension}`; // Use the full file name with the original extension
      document.body.appendChild(a);
      a.click();
      a.remove();
      window.URL.revokeObjectURL(url);
                      ...
                      <div className="text-right">
                        ...
                        {order.status === "completed" && (
                          <button
                            onClick={() =>
                              handleDownload(
                                `${process.env.NEXT_PUBLIC_URL_ENDPOINT}/${product.imageUrl}`, // Original image URL
                                `image-${order._id?.toString().slice(-6)}` // File name without extension
                              )
                            }
                            className="btn btn-primary gap-1"
                          >
